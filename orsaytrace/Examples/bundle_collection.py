...
for mesh in [1, 2, 3, 4, 5]:
    ...
    for x in numpy.linspace(min_x, max_x, 50):
        for z in numpy.linspace(min_z, max_z, 50):
    ...
    if it==1:
        axes.hist2d(x_new_list[0], z_new_list[0], bins=25, range=[[min_x, max_x], [min_z, max_z]])
    else:
        [axes[i].hist2d(x_new_list[i], z_new_list[i], bins=25, range=[[min_x, max_x], [min_z, max_z]]) for i in
         range(it)]
